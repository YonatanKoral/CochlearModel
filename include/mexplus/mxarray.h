		...
		*/
		...
		static void assignTo(const mxArray* array,
			...
			if (mxIsComplex(array)) {
				mxComplexDouble *cxdouble = mxGetComplexDoubles(array);
				cxdouble = cxdouble + index;
				T real_part = static_cast<T>(cxdouble->real);
				T imag_part = static_cast<T>(cxdouble->imag);
		...
		*/
		...
		static void assignTo(const mxArray* array,
			...
			if (!mxIsComplex(array)) {
				...
				mxComplexDouble *cxdouble = mxGetComplexDoubles(array);
				//T* real_part = cxdouble->real;// reinterpret_cast<T*>(mxGetPr(array));
				//T* imag_part = cxdouble->imag;// reinterpret_cast<T*>(mxGetPi(array));
				value->resize(array_size);
				for (mwSize i = 0; i < array_size; ++i) {
					
					double mag = std::abs(std::complex<double>(
						static_cast<double>(cxdouble->real),
						static_cast<double>(cxdouble->imag)));
					cxdouble++;
